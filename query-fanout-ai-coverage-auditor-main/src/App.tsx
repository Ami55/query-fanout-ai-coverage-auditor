...
function sanitizeProject(p: any): AuditProject {
  ...
  const rawSavedActions = Array.isArray(p.actionPlan) ? removeLeakedDemoRows(p.actionPlan) : Array.isArray(p.actionItems) ? removeLeakedDemoRows(p.actionItems) : [];
  const actionPlan = rawSavedActions.map((item: ActionItem) => {
    const priorityLabel = String(item.priority || '').toLowerCase();
    const executionQueue: 'Do now' | 'Do next' | 'Monitor' =
      priorityLabel.includes('immediate') || priorityLabel.includes('high') ? 'Do now' :
      priorityLabel.includes('medium') ? 'Do next' : 'Monitor';
    return { ...item, executionQueue };
  });
