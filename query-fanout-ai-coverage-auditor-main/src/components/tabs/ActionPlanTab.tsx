    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {(['Do now', 'Do next', 'Monitor'] as const).map((queue) => (
          <button
            ...
            title={`Filter backlog to ${queue}`}
            className={`text-left rounded-xl border p-4 cursor-pointer transition-all hover:-translate-y-0.5 hover:shadow-md ${selectedQueue === queue
          ...
          >
            ...
            <span className={`inline-block mt-2 text-[10px] font-bold ${selectedQueue === queue ? 'text-teal-300' : 'text-blue-600'}`}>
              {selectedQueue === queue ? 'Showing this queue — click to clear' : 'Click to filter tasks'}
            </span>
          </button>
        ))}
      </div>
      ...
      {selectedQueue !== 'all' && (
        <div className="flex items-center justify-between rounded-xl bg-blue-50 border border-blue-200 px-4 py-2.5 text-xs text-blue-900">
          <span>Showing only <strong>{selectedQueue}</strong> tasks.</span>
          <button type="button" onClick={() => setSelectedQueue('all')} className="font-bold underline cursor-pointer">Show all tasks</button>
        </div>
      )}
